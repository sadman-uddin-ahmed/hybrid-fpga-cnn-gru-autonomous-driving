...
module streaming_conv2_maxpool_layer #(
    ...
    wire [7:0]        conv2_output_x;
    wire [7:0]        conv2_output_y;
    wire [7:0]        conv2_output_group_index;
    wire [1:0]        conv2_output_lane_index;
    wire [7:0]        conv2_output_channel_index;
    wire              conv2_output_valid;
    streaming_conv2_layer #(
        ...
        .PADDING_FIFO_DEPTH(PADDING_FIFO_DEPTH),
        ...
        .output_x(conv2_output_x),
        .output_y(conv2_output_y),
        .output_group_index(conv2_output_group_index),
        .output_lane_index(conv2_output_lane_index),
        .output_channel_index(conv2_output_channel_index),
    ...
    streaming_maxpool_2x2 #(
        ...
        .clk(clk),
        .reset(reset),
        .input_value(conv2_output_value),
        .input_x(conv2_output_x),
        .input_y(conv2_output_y),
        .input_channel_index(conv2_output_channel_index),
        .input_valid(conv2_output_valid),
    ...
    initial begin
        $display(
            "ACTIVE RTL: streaming_conv2_maxpool_layer CANDIDATE-A V1"
        );
        if (INPUT_CHANNELS != 16) begin
            $display(
                "WARNING: streaming_conv2_maxpool_layer is intended for 16 Conv2 input channels."
            );
        end
        if (OUTPUT_CHANNELS != 32) begin
            $display(
                "WARNING: streaming_conv2_maxpool_layer is intended for 32 Conv2 output channels."
            );
        end
        if ((OUTPUT_CHANNELS % 4) != 0) begin
            $display(
                "ERROR: streaming_conv2_maxpool_layer OUTPUT_CHANNELS must be divisible by four."
            );
        end
        if (
            (IMAGE_WIDTH % 2) != 0 ||
            ...
            $display(
                "ERROR: streaming_conv2_maxpool_layer requires even Conv2 spatial dimensions."
            );
        end
    end
endmodule