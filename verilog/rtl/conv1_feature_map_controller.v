...
module conv1_feature_map_controller (
    input  wire clk,
    input  wire rst,
    input  wire start,
    output reg  done
    ...
    localparam OUTPUT_CHANNELS  = 16;
    localparam OUTPUT_HEIGHT    = 64;
    localparam OUTPUT_WIDTH     = 64;
    localparam INPUT_SIZE       = 3 * 64 * 64;
    localparam WEIGHT_SIZE      = 16 * 3 * 3 * 3;
    localparam BIAS_SIZE        = 16;
    localparam OUTPUT_SIZE      = 16 * 64 * 64;
    ...
    reg [4:0] output_channel_index;
    reg [6:0] output_row_index;
    reg [6:0] output_column_index;
    reg start_pixel_mac;
    wire pixel_mac_done;
    wire [1:0] mac_input_channel_index;
    ...
    reg signed [7:0]  conv1_output_memory [0:OUTPUT_SIZE-1];
    conv1_pixel_mac_controller pixel_mac_controller (
        .clk(clk),
        .rst(rst),
                ...
                STATE_STORE: begin
                    output_memory_index =
                        get_output_index(output_channel_index,
                                         output_row_index,
                                         output_column_index);
                    conv1_output_memory[output_memory_index] <= pixel_quantized_output;
