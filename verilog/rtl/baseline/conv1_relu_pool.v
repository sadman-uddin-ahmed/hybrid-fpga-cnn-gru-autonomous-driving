...
module conv1_relu_pool_model (
    input  wire clk,
    input  wire rst,
    input  wire start,
    output reg  done
);
    // Conv1 dimensions(Input  : 3 x 64 x 64, Weight : 16 x 3 x 3 x 3, Output : 16 x 32 x 32 after Conv + ReLU + MaxPool)
    localparam IN_C   = 3;
    localparam IN_H   = 64;
    localparam IN_W   = 64;
    localparam OUT_C  = 16;
    localparam K      = 3;
    localparam CONV_H = 64;
    localparam CONV_W = 64;
    localparam POOL_H = 32;
    localparam POOL_W = 32;
    localparam IMG_SIZE    = 3 * 64 * 64;       // 12288
    localparam W_SIZE      = 16 * 3 * 3 * 3;    // 432
    localparam B_SIZE      = 16;
    localparam OUT_SIZE    = 16 * 32 * 32;      // 16384
    // scale_acc_to_out = 0.0012125467105459956
    // Q30 multiplier = round(scale * 2^30) = 1301962
    localparam integer SCALE_MULT  = 1301962;
    ...
    // Internal full-resolution quantized Conv1 output before pooling
    reg signed [7:0] conv_relu_q [0:OUT_C*CONV_H*CONV_W-1];
    integer oc, ic, r, c, kr, kc;
    integer rr, cc;
    integer img_idx, w_idx, conv_idx, out_idx;
    integer pr, pc;
    integer acc;
    integer scaled;
    integer max_val;
    ...
    function integer IMG_INDEX;
        input integer ch;
        input integer row;
        input integer col;
        begin
            IMG_INDEX = (ch * IN_H * IN_W) + (row * IN_W) + col;
        end
    endfunction
    ...
    // Main bit-true Conv1 -> ReLU -> Quantize -> MaxPool model
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (start) begin
                ...
                // 1. Conv1 + Bias + ReLU + Quantization
                                ...
                                for (kr = 0; kr < K; kr = kr + 1) begin
                                    for (kc = 0; kc < K; kc = kc + 1) begin
                                        ...
                                        if ((rr >= 0) && (rr < IN_H) &&
                                            ...
                                            img_idx = IMG_INDEX(ic, rr, cc);
                                            w_idx   = W_INDEX(oc, ic, kr, kc);

                                            acc = acc + (img_mem[img_idx] * w_mem[w_idx]);
                                        end
                                    end
                                end
                            end
